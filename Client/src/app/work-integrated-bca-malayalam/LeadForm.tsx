...
function validate(values: FormState): FormErrors {
  ...
  if (!values.name.trim()) {
    errors.name = "പേര് ഇടൂ";
  } else if (values.name.trim().length < 2) {
    errors.name = "പേര് കുറച്ച് short ആണല്ലോ";
  }

  const digits = values.phone.replace(/\D/g, "");
  if (!values.phone.trim()) {
    errors.phone = "Number ഇടൂ";
  } else if (digits.length < 10 || digits.length > 13) {
    errors.phone = "Valid number ആകണം";
  }

  const marksNum = Number(values.marks);
  if (!values.marks.trim()) {
    errors.marks = "+2 Marks ഇടൂ";
  } else if (Number.isNaN(marksNum) || marksNum < 0 || marksNum > 100) {
    errors.marks = "0 മുതൽ 100 വരെ";
  }

  const yearNum = Number(values.passoutYear);
  const currentYear = new Date().getFullYear();
  if (!values.passoutYear.trim()) {
    errors.passoutYear = "Year ഇടൂ";
  } else if (
    Number.isNaN(yearNum) ||
    yearNum < 2000 ||
    yearNum > currentYear + 2
  ) {
    errors.passoutYear = `2000 മുതൽ ${currentYear + 2} വരെ`;
  }
...
function buildWhatsAppUrl(values: FormState) {
  const message =
    `Hi! Work-Integrated BCA program-നെ കുറിച്ച് അറിയണം.\n\n` +
    `പേര്: ${values.name.trim()}\n` +
    `Phone: ${values.phone.trim()}\n` +
    `+2 Marks: ${values.marks.trim()}%\n` +
    `+2 Pass വർഷം: ${values.passoutYear.trim()}`;
...
export default function LeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-3 md:gap-4">
      <div>
        <label htmlFor="lf-name" className={labelBase}>
          പേര് <span className="text-[#ef4444]">*</span>
        </label>
        <input
          id="lf-name"
          type="text"
      ...
      <div className="grid grid-cols-2 gap-2.5 md:gap-4">
        <div>
          <label htmlFor="lf-marks" className={labelBase}>
        ...
        <div>
          <label htmlFor="lf-year" className={labelBase}>
            +2 Pass വർഷം <span className="text-[#ef4444]">*</span>
          </label>
          <input
            id="lf-year"
            type="number"
      ...
      >
        {submitting ? (
          <span>WhatsApp തുറക്കുന്നു…</span>
        ) : (
          <>
            <svg
              width="20"
            ...
            Send ചെയ്യൂ — WhatsApp-ൽ ചാറ്റ് തുടരാം
          </>
        )}
      </button>

      <p className="hidden md:block text-center text-[12px] text-[#767e92] font-poppins mt-1">
        Submit ചെയ്താൽ WhatsApp open ആകും. Spam അയക്കില്ല!
      </p>
    </form>
  );
}
