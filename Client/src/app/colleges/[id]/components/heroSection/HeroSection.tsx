...
export default function HeroSection({ college }: any) {
  ...
  const bannerImage = college?.campusImages?.[0] || college?.logo || "/college-details-bg.png";
    ...
    >
      ...
      <div className="bg-white/92 backdrop-blur-sm shadow-[0_10px_40px_rgba(0,0,0,0.12)] relative sm:absolute mt-auto sm:mt-0 sm:bottom-[-88px] sm:left-1/2 sm:-translate-x-1/2 rounded-2xl px-3 py-3 sm:px-5 sm:py-4 w-[94%] max-w-[960px] z-10">
        ...
        <div className="flex flex-col gap-2.5 sm:hidden">
          {/* Row 1: Logo + Name/Location + Wishlist/Share */}
          <div className="flex items-start gap-3">
            <Image
              ...
              width={100}
              height={100}
              className="w-[48px] h-[48px] rounded-lg object-contain shrink-0 border border-gray-100"
            />
            <div className="flex-1 min-w-0">
              <h1 className="text-[#162447] text-[15px] leading-[19px] font-semibold line-clamp-2">
                {college?.name || "College Name"}
              </h1>
              <span className="mt-0.5 inline-flex items-center gap-1 text-[11px] text-[#767E92]">
                <LocationIcon width={12} height={12} fill="#767E92" />
                {college?.city}, {college?.state}
              </span>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button className="p-1.5 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors">
                <LoveIcon width={14} height={14} fill="#767E92" />
              </button>
              <button
                className="p-1.5 rounded-full border border-gray-200 hover:bg-gray-50 transition-colors"
                onClick={handleShare}
              >
                <ShareIcon width={14} height={14} fill="#767E92" />
              </button>
            </div>
          </div>
          ...
          {college?.accreditation?.length > 0 && (
            <div className="flex flex-wrap items-center gap-1">
              {college.accreditation.map((badge: any) => (
                <span
                  key={badge}
                  className="px-2 py-0.5 text-[10px] border border-gray-200 rounded-full text-[#4B4B4B] font-medium bg-white/60"
                >
                  {badge}
                </span>
              ))}
          ...
          {/* Actions: Apply + Compare inline, Download below */}
          <div className="flex items-center gap-2">
            <Button
              variant="primary"
              size="sm"
              className="flex-1 flex items-center justify-center gap-1.5 h-[36px] text-[13px] rounded-lg"
            ...
            <Button
              variant="outline"
              size="sm"
              className="flex-1 flex items-center justify-center h-[36px] text-[13px] rounded-lg"
          ...
          <button
            type="button"
            className="cursor-pointer flex justify-center items-center gap-1.5 bg-transparent border-none p-0"
          ...
          >
            <span className="flex items-center gap-1.5 text-[var(--primary-color)] text-[12px] hover:underline">
              Download Brochure
        ...
        {/* ========== DESKTOP LAYOUT ========== */}
        <div className="hidden sm:flex sm:flex-row sm:items-center gap-5">
          <Image
            src={college?.logo || "/minority.png"}
            alt={`${college?.name || "College"} logo`}
            width={100}
            height={100}
            className="w-[88px] h-[88px] md:w-[100px] md:h-[100px] rounded-xl object-contain shrink-0"
          />
          <div className="flex-1 flex flex-col gap-3">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 md:gap-3 shrink-0">
                <Button variant="outline" size="sm" className="px-3 py-2 text-sm">
                  <LoveIcon width={16} height={16} fill="currentColor" /> Wishlist
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  ...
                  onClick={handleShare}
                >
                  <ShareIcon width={16} height={16} fill="currentColor" /> Share
                </Button>
              </div>
            </div>
            ...
            {college?.accreditation?.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                {college.accreditation.map((badge: any) => (
                  <span
                    key={badge}
                    className="px-3 py-1.5 text-xs md:text-sm border border-gray-200 rounded-full text-[#4B4B4B] font-medium"
                  >
                    {badge}
                  </span>
                ))}
            ...
            <div className="flex items-center gap-[14px]">
              ...
              <button
                type="button"
                className="ml-2 cursor-pointer flex items-center gap-2 bg-transparent border-none p-0"
                aria-label={`Download brochure for ${college?.name || "this college"}`}
                onClick={() => {
                  if (college?.brochure) {
                    const link = document.createElement("a");
              ...
              >
                <span className="flex items-center gap-2 text-[var(--primary-color)] text-base hover:underline">
                  Download Brochure
